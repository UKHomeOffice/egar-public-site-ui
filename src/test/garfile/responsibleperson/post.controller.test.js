  ...
  it('should render with validation messages if given name is too long', async () => {
    ...
    callController().then(() => {
      ...
      expect(res.render).to.have.been.calledWith('app/garfile/responsibleperson/index', {
        ...
        fixedBasedOperatorOptions,
        errors: [
          new ValidationRule(validator.isValidStringLength, 'responsibleGivenName', req.body.responsibleGivenName, `Given name must be ${MAX_STRING_LENGTH} characters or less`),
        ],
      });
    });
  });
  ...
  it('should render with validation messages if no FBO selection is made', async () => {
    ...
    callController().then(() => {
      test.each([
        [null],
        [''],
      ])('assigns value to req.body.fixedBasedOperator', (reqBodyFixedBasedOperator) => {
        req.body.fixedBasedOperator = reqBodyFixedBasedOperator;
        ...
        expect(res.render).to.have.been.calledWith('app/garfile/responsibleperson/index', {
          ...
          fixedBasedOperatorOptions,
          errors: [
            fixedBasedOperatorValue !== 'Other' ? new ValidationRule(validator.notEmpty, 'fixedBasedOperator', req.body.fixedBasedOperator, `Select the role of the responsible person.`) : new ValidationRule(validator.notEmpty, 'fixedBasedOperatorAnswer', req.body.fixedBasedOperatorAnswer, `Enter your responsible person role.`),
          ],
        });
      });
    });
