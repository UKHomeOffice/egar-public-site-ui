...
describe('bulkAdd class', () => {
  ...
  it('should return nothing if required ids do not match', async () => {
    sinon
      ...
      .resolves(JSON.stringify([{ personId: 'PERSON-3' }]));

    await getDetailsByIds(['PERSON-1', 'PERSON-2'], 'USER-ID-1').then(
      (result) => {
        expect(personApi.getPeople).to.have.been.calledWith(
          'USER-ID-1',
          'individual'
        );
  ...
  it('should reject if person api rejects', async () => {
    sinon
      .stub(personApi, 'getPeople')
      .rejects('personApi.getPeople Example Reject');

    await getDetailsByIds(['PERSON-1', 'PERSON-2'], 'USER-ID-1')
      .then()
      .catch((err) => {
        expect(personApi.getPeople).to.have.been.calledWith(
          'USER-ID-1',
