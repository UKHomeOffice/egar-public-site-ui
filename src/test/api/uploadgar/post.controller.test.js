  ...
  describe('validations', () => {
    ...
    it('should return error if departure date too far in advance', async () => {
      ...
      callController().then(() => {
        expect(req.session.save).to.have.been.called;
        expect(req.session.failureMsg).to.eql([
          new ValidationRule(validator.dateNotTooFarInFuture, '', '2022-07-30', 'Departure date cannot be more than 1 month in the future'),
        ]);
        expect(res.redirect).to.have.been.calledWith('/garfile/garupload');
      });
    });
    ...
    it('should return error if arrival date too far in advance', async () => {
      ...
      callController().then(() => {
        expect(req.session.save).to.have.been.called;
        expect(req.session.failureMsg).to.eql([
          new ValidationRule(validator.dateNotTooFarInFuture, '', '2022-07-30', 'Arrival date cannot be more than 1 month in the future'),
        ]);
        expect(res.redirect).to.have.been.calledWith('/garfile/garupload');
      });
    });
