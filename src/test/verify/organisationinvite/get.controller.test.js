...
describe('Verify Organisation Invite Get Controller', () => {
  ...
  const oneLoginAuthUrl =
    settings.ONE_LOGIN_SHOW_ONE_LOGIN === true
      ? 'https://onelogin?code=123&state=state'
      : '';
  const pathName =
    settings.ONE_LOGIN_SHOW_ONE_LOGIN === true
      ? '/verify/invite/onelogin'
      : '/verify/invite/';
  ...
  beforeEach(() => {
    ...
    verifyUserServiceStub = sinon.stub(
      verifyUserService,
  ...
  it('should clear the cookie and set a hashed token before render', async () => {
    ...
    expect(res.render).to.have.been.calledOnceWithExactly(
      'app/verify/organisationinvite/index',
      { pathName, oneLoginAuthUrl }
    );
  });
});
