...
describe('Verify MFA Post Controller', () => {
  ...
  describe('token api resolves', () => {
    it('should return message if valid but token api update rejects', async () => {
      ...
      sinon
        .stub(tokenApi, 'updateMfaToken')
        .rejects('tokenApi.updateMfaToken Example Reject');
      sinon.stub(userApi, 'getDetails');
      settings.MFA_TOKEN_LENGTH = 20;
      const cookie = new CookieModel(req);
              ...
              errors: [
                {
                  identifier: 'mfaCode',
                  message:
                    'There was a problem verifying your token. Try again.',
                },
              ],
            }
          );
    ...
    it('should return message if valid token api resolves but user api rejects', async () => {
      ...
      sinon
        .stub(userApi, 'getDetails')
        .rejects('userApi.getDetails Example Reject');
      settings.MFA_TOKEN_LENGTH = 20;
      const cookie = new CookieModel(req);
    ...
    it('should successfully login', async () => {
      ...
      callController()
        .then(() => {
          ...
          expect(
            tokenApi.updateMfaToken
          ).to.have.been.called.calledOnceWithExactly(
            'example@somewhere.com',
            123456
          );
          expect(userApi.getDetails).to.not.have.been.called;
        ...
        .then(() => {
          ...
          expect(userApi.getDetails).to.have.been.calledOnceWithExactly(
            'example@somewhere.com'
          );
