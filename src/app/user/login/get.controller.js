...
const isUserAuthenticated = (userSessionObject) => {
  ...
  return !!(
    userSessionObject.dbId &&
    userSessionObject.vr &&
    userSessionObject.rl
  );
};
    ...
    .then(async (userData) => {
      ...
      switch (true) {
        ...
        case emailMatches && userData.oneLoginSid === null:
          ...
          return new Promise((resolve, reject) =>
            userApi
              .updateEmailOrOneLoginSid(userData.email, { oneLoginSid })
              .then((e) => {
                userData.oneLoginSid = oneLoginSid;
                return resolve(userData);
              })
          );
        case !oneLoginSidMatches &&
          emailMatches &&
          userData.oneLoginSid !== null:
          // condition: User had SID in our DB that doesn't match the one from ONELOGIN. Email matches however.
          logger.error(
            'login error: User SID does not match but email matches.'
          );
          return { redirect: redirectErrorPage(req, res, 'login-error') };
        default:
          logger.error(
            'User Id not found or email not verified during onelogin flow.'
          );
...
const setUserCookies = (cookie, userData) => {
  const { email, firstName, lastName, userId, role, organisation, state } =
    userData;
