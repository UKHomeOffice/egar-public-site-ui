...
const {
  getUserInviteToken,
} = require('../../../common/services/verificationApi');
...
async function handleGivenNameSubmission(req, res) {
  ...
  try {
    ...
    try {
      ...
      const { email, sub } =
        await oneLoginApi.getUserInfoFromOneLogin(accessToken);
...
async function handleConfirmNameSubmission(req, res) {
  if (!req.body.nameConfirmDeclaration) {
    ...
    return [
      Outcome.DECLARATION_NOT_CHECKED,
...
module.exports = async (req, res) => {
  ...
  const [outCome, data, redirect] = await stepHandlerMapping[req.session.step](
    req,
    res
  );
