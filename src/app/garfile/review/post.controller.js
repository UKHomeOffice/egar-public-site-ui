...
module.exports = async (req, res) => {
  ...
  const resubmit0TLink = req.body.resubmitFor0TLink
    ? req.body.resubmitFor0TLink
    : 'no';

  const isResubmit0T = [
    'resubmit0T',
    'resubmit0TOnSummaryPage',
    'resubmitFor0T',
  ].includes(resubmit);
  ...
  if (resubmit === 'resubmit0TOnSummaryPage') {
    ...
    if (isRequiresPassengerCheck && !isAnAllMilitaryFlight) {
      ...
      res.redirect(
        `/garfile/amg/checkin?resubmitted=${resubmit0TLink}${initialSubmit}`
      );
    } else {
