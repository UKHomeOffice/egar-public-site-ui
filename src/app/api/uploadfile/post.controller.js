    ...
    garApi
      ...
      .then((gars) => {
        ...
        if (parsedGars.items.length >= MAX_NUM) {
          logger.info(
            `Number of supporting docs exceeds the limit: ${MAX_NUM}, gar:${garId}`
          );
      ...
      .catch((err) => {
        logger.error(
          'Unknown error whilst determining GAR supporting documents file sizes'
        );
        logger.error(err);
        reject(err);
      });
...
module.exports = (req, res) => {
  ...
  exceedFileNumSizeLimit(req.file.size, req.body.garid)
    .then((result) => {
      ...
      logger.debug(
        `In Upload File Service. Uploaded File: ${req.file.originalname}`
      );
      const mimeType = fileType(req.file.buffer);
      logger.info(
        `Detected uploaded file mimetype as: ${JSON.stringify(mimeType)}`
      );
