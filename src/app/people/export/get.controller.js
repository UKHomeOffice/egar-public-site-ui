...
const createArrayCsvStringifier =
  require('csv-writer').createArrayCsvStringifier;
...
const writePeopleAsCSVtoResponse = (res, people) => {
  ...
  res.setHeader(
    'Content-disposition',
    'attachment; filename=people-' + dateSlug + '.csv'
  );
...
module.exports = (req, res) => {
  ...
  logger.debug(
    `In people export get controller, user ${userId} is exporting data.`
  );

  personApi.getPeople(userId, 'individual').then((values) => {
