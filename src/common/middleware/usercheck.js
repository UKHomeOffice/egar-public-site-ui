...
const checkUserInCookie = (cookie) => {
  const userDbIdNotSet =
    cookie.getUserDbId() === undefined || cookie.getUserDbId() === null;
  const userVerifiedNotSet =
    cookie.getUserVerified() === undefined ||
    cookie.getUserVerified() === null ||
    !cookie.getUserVerified();
  const userRoleNotSet =
    cookie.getUserRole() === undefined || cookie.getUserRole() === null;
...
module.exports = (req, res, next) => {
  ...
  redirectUrl !== '/welcome/index'
    ? cookie.setRedirectUrl(redirectUrl)
    : cookie.setRedirectUrl('');
  ...
  if (req.headers.referer === undefined) {
    logger.info(
      'About to redirect to login because the referer in headers is not set'
    );
    res.redirect('/welcome/index');
  } else {
    logger.info(
      'About to redirect to login because the user is not set in the cookie'
    );
    res.redirect('/welcome/index');
  }
};
