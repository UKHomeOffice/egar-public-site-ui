...
function findAirportForCode(airportCode) {
  ...
  const matches = airportCodes.filter((item) =>
    [item.id, item.id2].includes(airportCode)
  );

  if (matches.length > 1) {
    throw new Error(
      `airport code ${airportCode} matched more than one airport`
    );
  }
...
function isJourneyUKInbound(departureCode, arrivalCode) {
  ...
  if (
    departureAirfield &&
    isAirportBritishOrCrownDependency(departureAirfield)
  ) {
