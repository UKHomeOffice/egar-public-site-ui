document.addEventListener('DOMContentLoaded', () => {
  const datePrefixArr = ['departure', 'arrival', 'dob', 'expiry'];

  const datePrefix = datePrefixArr
    .map((prefix) => document.getElementById(`date-prefix-${prefix}`))
    .filter((pr) => pr)
    .map((pr) => pr.dataset.prefix);

  datePrefix.forEach((prefix) => {
    {
